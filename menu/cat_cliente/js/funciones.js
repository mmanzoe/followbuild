...
function inicioEventos(){
	...
	$('#grabar').submit(grabaCliente);
	$('.resultados').on("click",".eliminar",eliminaCliente);
	listaCliente();
	$('.resultados').on('click','.actualizar', recuperaCliente);
...
function recuperaCliente(event){
	event.preventDefault();
