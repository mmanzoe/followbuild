...
$validapago = "SELECT fac_en.monto, (SELECT SUM(valor) FROM detalle_pago_cliente WHERE id_factura='".$idfactura."') as pagado, (fac_en.monto - (SELECT SUM(valor) FROM detalle_pago_cliente WHERE id_factura='".$idfactura."') ) as diferencia FROM factura_cliente_encabezado as fac_en WHERE fac_en.id ='".$idfactura."'";
$resulpago = mysqli_query($conexion, $validapago);
$fila = mysqli_fetch_assoc($resulpago);

if ($fila['diferencia'] == ''){
    $diferencia = $fila['monto'];
}else{
